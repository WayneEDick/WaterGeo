...
def _p2_params(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "delta_mult": float(p.get("delta_mult", 0.60)),
...
def _vertical_companion_speck(P: CC, S: CC, th: Dict[str, Any], xh_d: float) -> bool:
    if P.kind != CCKind.CHAR_LIKE:
        return False
    if S.kind != CCKind.SPECK:
        return False

    # scale by xh_d (dominant), since CC has no per-cc xh in current codebase
    eps = th["eps_mult"] * xh_d
    delta = th["delta_mult"] * xh_d
    a = th["a_mult"] * xh_d
    d = th["d_mult"] * xh_d
...
def v_graph(ctx: Context, cfg: Dict[str, Any]) -> None:
    """Phase 2A: V_Graph (vertical stacks): attaches SPECK debris to CHAR_LIKE planets."""
    ccs: List[CC] = ctx.get("ccs_g3", None)
    if not isinstance(ccs, list):
    ...
    for P in planets:
        ...
        for sid in cand_ids:
            ...
            if _vertical_companion_speck(P, S, th, xh_d):
                gy = _gap_y_cc(P, S)
                Scx, Scy = _cc_center_xy(S)
                dx = abs(Scx - Pcx)
                if Scy < Pcy:
...
def debug_render_hg_boxes_only(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    # Need VBox lookup for marking members of H_RUN tokens
    VBoxMap: Dict[int, VBox] = ctx.get("vbox_by_id", None)
    if not isinstance(VBoxMap, dict):
        raise ValueError("debug_render_hg_boxes_only: ctx['vbox_by_id'] missing (run v_graph first)")
    ...
    rgb = np.full((H * upscale, W * upscale, 3), 255, dtype=np.uint8)
    thickness = max(3, 2 * int(upscale))
    fill_alpha = float(cfg.get("outputs", {}).get("hg_boxes_only_fill_alpha", 1.0) or 1.0)
    fill_alpha = max(0.0, min(1.0, fill_alpha))
    overlay = rgb.copy()

    # token color map (reuse existing palette)
    token_rgb = {
        "H_RUN": (140, 130, 85),            # char_like olive
        "WD_SPACE": (210, 210, 210),
        "WIDE_SPACE": (180, 180, 180),
        "BIG_SYM_TOKEN": (110, 110, 200),
        "FLOATER_BLOB_TOKEN": (200, 120, 120),
    }

    for tok in tokens:
        col = token_rgb.get(tok.kind, (210, 210, 210))
        ...
        # Draw token rectangles
        if fill_alpha > 0.0:
            cv2.rectangle(overlay, (x0, y0), (x1, y1), bgr, thickness=-1)
        cv2.rectangle(overlay, (x0, y0), (x1, y1), bgr, thickness=thickness)

        # Add "s"/"." markers for each VBox in an H_RUN token
        if tok.kind == "H_RUN" and getattr(tok, "members", None):
            for j, vid in enumerate(tok.members):
                B = VBoxMap.get(vid, None)
                if B is None:
                    continue

                cx = int(B.cx * upscale)
                cy = int(B.cy * upscale)
                if not (0 <= cx < rgb.shape[1] and 0 <= cy < rgb.shape[0]):
                    continue
    ...
    out_path = out_dir / f"{stem}_hg_boxes_only.png"
    cv2.imwrite(str(out_path), rgb)
