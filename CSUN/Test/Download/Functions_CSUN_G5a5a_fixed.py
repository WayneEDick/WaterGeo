...
def g5a5a_attach_accents(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    # Starter tolerances (conservative)
    eps_x = 0.20 * xh_d
    max_gap_above = 0.80 * xh_d
    max_child_h_mult = 0.80
    max_child_w_mult = 0.80
    min_ov_x_mult = 0.25
    ...
    def candidate_child(tok: Token) -> bool:
        if tok.kind in {"H_RUN", "WD_SPACE", "WIDE_SPACE"}:
            return False
    ...
    for child in children:
        ...
        for parent in parents:
            ...
            # child must be modest relative to parent
            if float(child.h) > max_child_h_mult * float(parent.h):
                continue
            if float(child.w) > max_child_w_mult * float(parent.w):
                continue
    ...
    ctx["g5a5a"] = {
        ...
        "max_child_h_mult": max_child_h_mult,
        "max_child_w_mult": max_child_w_mult,
