...
class Token:
    ...
    # ---- G5a.5 attachment annotations ----
    parent_token_id: int = 0
    child_role: str = ""
    can_start_line: bool = True
# -----------------------------
...
# ============================================================
# G5a.5a: Residual accent / host attachment
# ============================================================

def g5a5a_attach_accents(ctx: Context, cfg: Dict[str, Any]) -> None:
    """
    G5a.5a: attach residual accent-like child tokens to parent hosts.

    Purpose:
      - suppress false line starts from accent children that V_Graph did not absorb
      - record parent/child structure for later reading-order and semantic work

    Policy:
      - This pass does NOT merge tokens or change token geometry.
      - It only annotates child tokens:
            parent_token_id
            child_role = "accent_above"
            can_start_line = False
      - Parent tokens remain unchanged.

    IMPORTANT (revised):
      - We do NOT require the child to be a FLOATER.
      - Residual accents often survive as tiny inline H_RUN tokens.
      - So candidate children are small inline non-space tokens, excluding BigSym hosts.
    """
    tokens: List[Token] = ctx.get("tokens_g5a", None)
    if not isinstance(tokens, list):
        raise ValueError("g5a5a_attach_accents: ctx['tokens_g5a'] missing (run g5a_classify_tokens first)")

    g4 = ctx.get("g4", None)
    if not isinstance(g4, dict) or "xhd" not in g4:
        raise ValueError("g5a5a_attach_accents: ctx['g4']['xhd'] missing (run g4_page_constants first)")

    xh_d = float(g4["xhd"])

    # Stable token ids + default annotations
    for i, tok in enumerate(tokens, 1):
        if not getattr(tok, "token_id", 0):
            tok.token_id = i
        tok.parent_token_id = int(getattr(tok, "parent_token_id", 0) or 0)
        tok.child_role = str(getattr(tok, "child_role", "") or "")
        if tok.parent_token_id == 0 and tok.child_role == "":
            tok.can_start_line = True
        else:
            tok.can_start_line = bool(getattr(tok, "can_start_line", True))

    # Revised tolerances: child is a small inline token above a host.
    eps_x = 0.20 * xh_d
    max_gap_above = 0.90 * xh_d
    max_child_h = 0.80 * xh_d
    max_child_w = 1.20 * xh_d
    min_ov_x_mult = 0.25

    def x_overlap(a: Token, b: Token) -> float:
        return max(0.0, min(float(a.r), float(b.r)) - max(float(a.l), float(b.l)))

    def candidate_child(tok: Token) -> bool:
        if tok.kind in {"WD_SPACE", "WIDE_SPACE"}:
            return False
        if tok.parent_token_id:
            return False
        if not bool(getattr(tok, "inline", False)):
            return False
        if getattr(tok, "role", "") == "BigSym":
            return False
        if float(tok.h) > max_child_h:
            return False
        if float(tok.w) > max_child_w:
            return False
        return True

    def candidate_parent(tok: Token) -> bool:
        if tok.kind == "H_RUN":
            return True
        if getattr(tok, "role", "") == "BigSym" and bool(getattr(tok, "inline", False)):
            return True
        return False

    attachments: List[Dict[str, Any]] = []

    children = [tok for tok in tokens if candidate_child(tok)]
    parents = [tok for tok in tokens if candidate_parent(tok)]

    children.sort(key=lambda z: (float(z.t), float(z.l), int(getattr(z, "token_id", 0))))
    parents.sort(key=lambda z: (float(z.t), float(z.l), int(getattr(z, "token_id", 0))))

    for child in children:
        best = None  # (score tuple, parent)

        for parent in parents:
            if int(parent.token_id) == int(child.token_id):
                continue

            # child must lie just above parent
            gap_above = float(parent.t) - float(child.b)
            if gap_above < 0.0 or gap_above > max_gap_above:
                continue

            # horizontal centering / containment
            if not (float(parent.l) - eps_x <= float(child.cx) <= float(parent.r) + eps_x):
                continue

            ovx = x_overlap(child, parent)
            if ovx < min_ov_x_mult * min(float(child.w), float(parent.w)):
                continue

            center_dx = abs(float(child.cx) - float(parent.cx))
            score = (gap_above, center_dx, -ovx, float(parent.t), float(parent.l))
        ...
        parent = best[1]
        child.parent_token_id = int(parent.token_id)
        child.child_role = "accent_above"
        child.can_start_line = False

        attachments.append({
            "parent_token_id": int(parent.token_id),
            "child_token_id": int(child.token_id),
            "relation": "accent_above",
            "parent_kind": str(parent.kind),
            "child_kind": str(child.kind),
            "gap_above": round(float(parent.t) - float(child.b), 3),
            "center_dx": round(abs(float(child.cx) - float(parent.cx)), 3),
        })

    print(f"G5a.5a accent candidates: {len(children)}")
    print(f"G5a.5a attachments made: {len(attachments)}")

    ctx["tokens_g5a"] = tokens
    ctx["g5a5a"] = {
        "eps_x": eps_x,
        "max_gap_above": max_gap_above,
        "max_child_h": max_child_h,
        "max_child_w": max_child_w,
        "min_ov_x_mult": min_ov_x_mult,
        "accent_candidates": len(children),
        "attachments_made": len(attachments),
...
def g6a_fraction_blocks(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    eligible_non_space = [t for t in non_space if bool(getattr(t, "can_start_line", True))]
...
# ============================================================
# G5b: Stream starts + provisional streams
# ============================================================

def g5b_make_streams(ctx: Context, cfg: Dict[str, Any]) -> None:
    """
    G5b:
      1) detect candidate walls
      2) detect line starts by:
            no predecessor exists after nearest wall
      3) build provisional streams

    Output:
      - ctx["g5b_line_starts"] : list of dicts
      - ctx["g5b_streams"]     : list of dicts
    """
    page = ctx.get("page", None)
    if page is None:
        raise ValueError("g5b_make_streams: ctx['page'] missing")

    tokens: List[Token] = ctx.get("tokens_g5a", None)
    if not isinstance(tokens, list):
        raise ValueError("g5b_make_streams: ctx['tokens_g5a'] missing (run g5a_classify_tokens first)")

    g4 = ctx.get("g4", None)
    if not isinstance(g4, dict) or "xhd" not in g4:
        raise ValueError("g5b_make_streams: ctx['g4']['xhd'] missing")
    ...
    def nearest_wall_for(tok: Token, walls: List[Dict[str, Any]]) -> Dict[str, Any]:
        candidates = []
        for w in walls:
            if float(w["r"]) < float(tok.l) and not (float(w["b"]) < float(tok.t) or float(tok.b) < float(w["t"])):
                candidates.append(w)
        if not candidates:
            return walls[0]
        return max(candidates, key=lambda z: (float(z["r"]), float(z["t"])))

    def is_attachment_candidate(tok: Token, hosts: List[Token]) -> bool:
        # suppress false starts such as sigma limits
        for host in hosts:
            if int(host.token_id) == int(tok.token_id):
                continue
            if getattr(host, "role", "") != "BigSym":
                continue
            if bool(getattr(host, "inline", False)):
                continue

            ovx = x_overlap(tok, host)
            if ovx <= 0.25 * min(float(tok.w), float(host.w)):
                continue

            above_gap = float(host.t) - float(tok.b)
            below_gap = float(tok.t) - float(host.b)

            if -0.25 * xh_d <= above_gap <= 1.25 * xh_d:
                return True
            if -0.25 * xh_d <= below_gap <= 1.25 * xh_d:
                return True
        return False
    ...
    for tok in non_space:
        ...
        inline = bool(getattr(tok, "inline", False))
        if role == "Fence" or (role == "BigSym" and not inline):
            walls.append({
                "id": f"T{int(tok.token_id)}",
    ...
    hosts = [t for t in eligible_non_space if getattr(t, "role", "") == "BigSym"]

    for tok in sorted(eligible_non_space, key=lambda z: (float(z.t), float(z.l), int(z.token_id))):
        wall = nearest_wall_for(tok, walls)

        preds = []
        for s in eligible_non_space:
            if int(s.token_id) == int(tok.token_id):
                continue
            if float(s.r) < float(tok.l) and ov_y(s, tok) >= ov_y_min:
                preds.append(s)

        after_wall = [s for s in preds if float(s.r) > float(wall["r"])]

        if not after_wall and not is_attachment_candidate(tok, hosts):
            starts.append({
                "token_id": int(tok.token_id),
                "src_id": int(getattr(tok, "src_id", 0)),
                "wall_id": wall["id"],
                "reason": "no_predecessor_after_nearest_wall",
                "bbox": {
                    "l": int(round(float(tok.l))),
                    "t": int(round(float(tok.t))),
                    "r": int(round(float(tok.r))),
                    "b": int(round(float(tok.b))),
                    "w": int(round(float(tok.w))),
                    "h": int(round(float(tok.h))),
                },
            })

    # Provisional streams: group by H_Graph source row
    streams: List[Dict[str, Any]] = []
    by_src: Dict[int, List[Token]] = {}
    for tok in tokens:
        by_src.setdefault(int(getattr(tok, "src_id", 0)), []).append(tok)

    for src_id in sorted(by_src.keys()):
        row = sorted(by_src[src_id], key=lambda z: (float(z.l), float(z.t), int(getattr(z, "token_id", 0))))
        if not row:
            continue

        l = min(float(t.l) for t in row)
        r = max(float(t.r) for t in row)
        t0 = min(float(t.t) for t in row)
        b0 = max(float(t.b) for t in row)

        stream_items = []
        atom_count = 0
        gap_count = 0

        for j, tok in enumerate(row, 1):
            if tok.kind in {"WD_SPACE", "WIDE_SPACE"}:
                gap_count += 1
            else:
                atom_count += 1
            ...
            stream_items.append({
                "idx": j,
                "kind": str(tok.kind),
                "role": str(getattr(tok, "role", "Other")),
                "bbox": {
                    "l": int(round(float(tok.l))),
                    "t": int(round(float(tok.t))),
                    "r": int(round(float(tok.r))),
    ...
    ctx["g5b_line_starts"] = starts
    ctx["g5b_streams"] = streams
    ctx["g5b"] = {
        "xh_d": xh_d,
        "bin_height_mult": bin_height_mult,
        "ov_mult": ov_mult,
...
def debug_render_g5b_stream_boxes(ctx: Context, cfg: Dict[str, Any]) -> None:
    """
    Render provisional stream boxes on the page image.
    """
    page = ctx.get("page", None)
    if page is None:
        raise ValueError("debug_render_g5b_stream_boxes: ctx['page'] missing")

    streams = ctx.get("g5b_streams", None)
    if not isinstance(streams, list):
        raise ValueError("debug_render_g5b_stream_boxes: ctx['g5b_streams'] missing (run g5b_make_streams first)")

    base = ctx.get("imageGray", None)
    if not isinstance(base, np.ndarray) or base.ndim != 2:
        base = ctx.get("imageBin", None)
    if not isinstance(base, np.ndarray) or base.ndim != 2:
        raise ValueError("debug_render_g5b_stream_boxes: ctx['imageGray' or 'imageBin'] missing")
    ...
    for s in streams:
        ...
        cv2.rectangle(rgb, (x0, y0), (x1, y1), (0, 0, 255), thickness=thickness)
        cv2.putText(
            rgb,
            str(s["src_id"]),
            (x0 + 4, max(20, y0 - 6)),
    ...
    out_path = out_dir / f"{stem}_g5b_stream_boxes.png"
    cv2.imwrite(str(out_path), rgb)
    ctx["debug_g5b_stream_boxes_png"] = str(out_path)


def g5b_write_stream_report(ctx: Context, cfg: Dict[str, Any]) -> None:
    """
    Write:
      - <stem>_g5b_streams.yaml
      - <stem>_g5b_line_starts.yaml
    """
    page = ctx.get("page", None)
    if page is None:
        raise ValueError("g5b_write_stream_report: ctx['page'] missing")

    streams = ctx.get("g5b_streams", None)
    starts = ctx.get("g5b_line_starts", None)
    if not isinstance(streams, list):
        raise ValueError("g5b_write_stream_report: ctx['g5b_streams'] missing")
    if not isinstance(starts, list):
        raise ValueError("g5b_write_stream_report: ctx['g5b_line_starts'] missing")
    ...
    # streams YAML
    stream_lines: List[str] = ["g5b_streams:"]
    for s in streams:
        stream_lines.append(f"  - src_id: {s['src_id']}")
        stream_lines.append("    bbox:")
        for k in ["l", "t", "r", "b", "w", "h"]:
            stream_lines.append(f"      {k}: {s['bbox'][k]}")
        stream_lines.append(f"    atom_count: {s['atom_count']}")
        stream_lines.append(f"    gap_count: {s['gap_count']}")
        stream_lines.append(f"    warnings: {s['warnings']}")
        stream_lines.append("    stream:")
        for item in s["stream"]:
            stream_lines.append(f"      - idx: {item['idx']}")
            stream_lines.append(f"        kind: \"{item['kind']}\"")
            stream_lines.append(f"        role: \"{item['role']}\"")
            stream_lines.append("        bbox:")
            for k in ["l", "t", "r", "b", "w", "h"]:
                stream_lines.append(f"          {k}: {item['bbox'][k]}")

    stream_path = out_dir / f"{stem}_g5b_streams.yaml"
    stream_path.write_text("\n".join(stream_lines) + "\n", encoding="utf-8")
    ctx["g5b_stream_report_yml"] = str(stream_path)

    # line starts YAML
    start_lines: List[str] = ["g5b_line_starts:"]
    for s in starts:
        start_lines.append(f"  - token_id: {s['token_id']}")
        start_lines.append(f"    src_id: {s['src_id']}")
        start_lines.append(f"    wall_id: \"{s['wall_id']}\"")
        start_lines.append(f"    reason: \"{s['reason']}\"")
        start_lines.append("    bbox:")
        for k in ["l", "t", "r", "b", "w", "h"]:
            start_lines.append(f"      {k}: {s['bbox'][k]}")

    start_path = out_dir / f"{stem}_g5b_line_starts.yaml"
    start_path.write_text("\n".join(start_lines) + "\n", encoding="utf-8")
    ctx["g5b_line_starts_yml"] = str(start_path)
