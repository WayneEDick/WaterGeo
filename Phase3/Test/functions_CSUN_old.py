...
def g5a_classify_tokens(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    def classify_shape(tok: Token) -> str:
        # Only meaningful for non-text tokens (floaters); keep "-" otherwise.
        w = float(tok.w)
        h = float(tok.h)
    ...
    for tok in tokens:
        ...
        tok.length = int(len(tok.members) if tok.kind == "H_RUN" and tok.members else (1 if tok.kind == "FLOATER_BLOB_TOKEN" else 0))
        ...
        if tok.kind == "H_RUN":
            ...
            tok.role = "BLOCK_FLOATER"  # provisional; may become INLINE_FLOATER
            tok.shape = classify_shape(tok)
        else:
            tok.shape = "-"

    # Decide INLINE_FLOATER vs BLOCK_FLOATER using same-bin text support
    for k, toks in bins.items():
        ...
        for tok in toks:
            ...
            if best <= gap_x_max:
                tok.inline = True
                tok.role = "INLINE_FLOATER"
            else:
                tok.inline = False
                tok.role = "BLOCK_FLOATER"
