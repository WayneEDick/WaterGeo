...
def h_graph(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    def emit_h_run(member_ids: List[int]) -> None:
        ...
        tokens.append(Token(kind="H_RUN", l=l, r=r, t=t, b=b, w=w, h=h, cx=cx, cy=cy, xh=float(xh_d), members=list(member_ids)))
    ...
    def emit_blob_run(member_ids: List[int]) -> None:
        ...
        w = r - l; h = b - t
        cx = (l + r) / 2.0; cy = (t + b) / 2.0
        tokens.append(Token(kind="FLOATER_BLOB_TOKEN", l=l, r=r, t=t, b=b, w=w, h=h, cx=cx, cy=cy, xh=float(xh_d), members=list(member_ids)))
