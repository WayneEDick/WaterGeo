...
def mark_floaters(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    def is_floater(B: VBox, ids_in_bin: List[int]) -> bool:
        ...
        # Gate 2 shape
        is_tall_thin = (B.h > th["tall_mult"] * xh_d) and (B.w < th["thin_mult"] * xh_d)
        is_wide_flat = (B.w > th["wide_mult"] * xh_d) and (B.h < th["flat_mult"] * xh_d)
        is_tiny_speck = (B.h < th["speck_h_mult"] * xh_d) and (B.w < th["speck_w_mult"] * xh_d)
        ...
        # IMPORTANT (Wayne rule):
        # Hyphen / en-dash / em-dash / minus and similar punctuation glyphs are characters.
        # If G3 classified this box as CHAR_LIKE, do NOT demote it to a floater based on support.
        # (Structural bars/fences are already caught by is_tall_thin / is_wide_flat above.)
        if B.kind == CCKind.CHAR_LIKE:
            return False

        # Gate 3 support (only for non-char-like objects)
