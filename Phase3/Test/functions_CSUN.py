...
def debug_render_g3_boxes(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    if not isinstance(base, np.ndarray) or base.ndim != 2:
        raise ValueError(
            "debug_render_g3_boxes: ctx['imageGray' or 'imageBin'] missing (run load_normalize/binarize first)")
...
def debug_render_vg_boxes_only(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    for vb in vboxes:
        ...
        if cfg.get("outputs", {}).get("debug_draw_cc_box_centers", True):
            cx = int(vb.cx * upscale);
            cy = int(vb.cy * upscale)
...
def h_graph(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    def emit_atomic(kind: str, vid: int) -> None:
        B = vbox_by_id[vid]
        tokens.append(
            Token(kind=kind, l=B.l, r=B.r, t=B.t, b=B.b, w=B.w, h=B.h, cx=B.cx, cy=B.cy, xh=float(xh_d), members=[vid]))
...
def debug_render_hg_boxes_only(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    for i, tok in enumerate(tokens, start=1):
        ...
        x0 = int(tok.l * upscale);
        y0 = int(tok.t * upscale)
        x1 = int(tok.r * upscale);
        y1 = int(tok.b * upscale)
...
def g5a_classify_tokens(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    for tok in tokens:
        ...
        tok.length = int(len(tok.members) if tok.kind == "H_RUN" and tok.members else (
            1 if tok.kind in {"FLOATER_BLOB_TOKEN", "BIG_SYM_TOKEN"} else 0))
...
def g5a_write_token_report(ctx: Context, cfg: Dict[str, Any]) -> None:
    ...
    lines.append(
        f"phase2 multipliers: ov={g5a.get('ov_mult')}, support_gap={g5a.get('support_gap_mult')}, bin_h_mult={g5a.get('bin_height_mult')}")
    lines.append(
        f"shape mults: tall={g5a.get('tall_mult')}, thin={g5a.get('thin_mult')}, wide={g5a.get('wide_mult')}, flat={g5a.get('flat_mult')}")
    ...
    lines.append(
        "  idx  kind               role             inl shape            txt len  l   t   r   b   w   h   members")
    lines.append(
        "  ---  -----------------  ---------------  --- ---------------  --- ---  --- --- --- --- --- --- --------")
    ...
    for i, tok in enumerate(toks_sorted, 1):
        ...
        lines.append(
            f"  {i:3d}  {kind:<17}  {role:<15}  {inline:^3} {shape:<15}  {is_txt:^3} {length:3d}  {l:3d} {t:3d} {r:3d} {b:3d} {w:3d} {h:3d} {mem:8d}")
